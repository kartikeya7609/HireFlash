...
// ─── EmailJS Config (from frontend .env) ───────────────────────────────────
...
const OtpModal = ({ email, onVerified, onResend, loading, error, setError }) => {
  ...
  return (
    <div className="animate-fade-in text-center space-y-6 py-4">
      ...
      <div>
        <h2 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Enter Verification Code</h2>
      ...
      {error && (
        <div className="p-3 rounded-xl text-xs font-medium border border-rose-300 bg-rose-50 dark:bg-rose-950/30 text-rose-600 dark:text-rose-400">
          {error}
        </div>
      )}
      ...
      <div className="flex justify-center gap-2" onPaste={handlePaste}>
        {otp.map((digit, i) => (
          <input
            ...
            className="w-11 h-13 text-center text-xl font-bold rounded-xl border-2 border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950 text-slate-900 dark:text-white focus:border-amber-500 focus:ring-2 focus:ring-amber-500/30 outline-none transition-all"
          />
        ))}
      </div>
      ...
      <div className="space-y-3">
        <button
          ...
          className="flex items-center justify-center gap-2 w-full py-3.5 bg-slate-950 dark:bg-white text-white dark:text-slate-950 text-xs font-bold uppercase tracking-wider rounded-xl transition-all shadow-md hover:bg-amber-600 dark:hover:bg-amber-400 disabled:opacity-50 disabled:pointer-events-none"
        >
          {loading ? (
        ...
        <button
          type="button"
          onClick={onResend}
          className="flex items-center justify-center gap-1.5 mx-auto text-xs font-bold text-slate-500 dark:text-slate-400 hover:text-amber-600 dark:hover:text-amber-400 transition-colors"
        >
...
const Register = () => {
  ...
  // Pending registration payload saved for after OTP
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setApiError('Failed to send verification email. Check your EmailJS config in .env');
    } finally {
      setSendingOtp(false);
    }
  };
  ...
  // Called after OTP is confirmed correct → register Firebase + sync MongoDB
